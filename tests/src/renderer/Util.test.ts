...
describe("Util.joinLibraryRoute()", function () {
    ...
    test("Lots of slashes", () => {
        ...
        expect(joinLibraryRoute(route)).toBe(
            "/browse/arcadeflashpluginshockwave",
        );
    });
    test("Double dots", () => {
        const route: string = "..";
...
describe("Util Renderer Various", () => {
    ...
    const mockSocket: SharedSocket<WebSocket> = jest.genMockFromModule(
        "@shared/back/SharedSocket",
    );
    ...
    test("Get Game Image URL", () => {
        expect(getGameImageURL("test", "abcd")).toBe(
            "http://mockserver/images/test/ab/cd/abcd.png",
        );
    });

    test("Get Platform Image URL", () => {
        expect(getPlatformIconURL("test")).toBe(
            "http://mockserver/logos/test.png",
        );
    });
    ...
    test("Is Exodos Path Valid", async () => {
        await expect(
            isExodosValidCheck(
                path.join(STATIC_PATH, "Util_Renderer", "ExodosPath"),
            ),
        ).resolves.toBeTruthy();
