...
import {
    generateExtrasAddApp,
    generateMessageAddApp,
} from "@shared/curate/parse";

describe("parseCurationFormat()", function () {
    test("Empty string", () => {
        expect(parseCurationFormat(tokenizeCurationFormat(""))).toEqual({});
    });
    test("List", () => {
        expect(
            parseCurationFormat(
                tokenizeCurationFormat(
                    "list:\n" + "- some\n" + "- boring\n" + "- stuff",
                ),
            ),
