...
export function main(init: Init): void {
        ...
        .then<WebSocket>(() =>
          timeout(
            new Promise((resolve, reject) => {
              ...
              ws.onclose = () => {
                reject(new Error("Failed to authenticate connection to back."));
              };
              ws.onerror = (event) => {
                reject(event.error);
              };
  ...
  function createMainWindow(): BrowserWindow {
    if (!state.preferences) {
      throw new Error("Preferences must be set before you can open a window.");
    }
    if (!state.config) {
      throw new Error("Configs must be set before you can open a window.");
    }
