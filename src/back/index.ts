...
import { PlaylistFile } from "./playlist/PlaylistFile";
...
function initExodosInstalledGamesWatcher() {
	...
	console.log(`Initializing installed games watcher with ${gamesPath} path...`);
...
const startFileServer = async (acceptRemote: boolean): Promise<number> =>
	new Promise<number>((resolve) => {
		...
		function tryListen() {
			if (port++ < maxPort) {
				state.fileServer.listen(port, acceptRemote ? undefined : "localhost");
			} else {
				done(
					new Error(
