  ...
  state.themeWatcher.on("ready", () => {
    ...
    state.themeWatcher.on("remove", (filename: string, offsetPath: string) => {
      state.themeQueue.push(() => {
        const item = findOwner(filename, offsetPath);
        if (item) {
          if (item.entryPath === path.join(offsetPath, filename)) {
      ...
      state.themeQueue.push(async () => {
        ...
        if (item) {
          ...
          if (
            ...
            file.isFile() &&
            (offsetPath === "" ||
              (offsetPath === folderName && filename === themeEntryFilename))
          ) {
    ...
    function findOwner(filename: string, offsetPath: string) {
      if (offsetPath) {
        ...
        return state.themeFiles.find(
          (item) => item.entryPath === filename || item.basename === filename
        );
      }
    }
  });
    ...
    case BackIn.LAUNCH_COMMAND:
      {
        ...
        const appPath = fixSlashes(
          path.join(path.resolve(state.config.exodosPath), reqData.path)
        );
          ...
          if (reqData.dupeImages) {
            ...
            try {
              if (await pathExists(oldLogoPath)) {
                await fs.promises.mkdir(path.dirname(newLogoPath), {
                  recursive: true,
                });
            ...
            try {
              if (await pathExists(oldScreenshotPath)) {
                await fs.promises.mkdir(path.dirname(newScreenshotPath), {
                  recursive: true,
                });
    ...
    case BackIn.EXPORT_GAME:
      {
        ...
        if (
          await pathExists(
            reqData.metaOnly ? path.dirname(reqData.location) : reqData.location
          )
        ) {
          const game = findGame(reqData.id);
          if (game) {
        ...
        if (dif) {
          if (
            (typeof dif.currentLanguage !== "undefined" &&
              dif.currentLanguage !== state.preferences.currentLanguage) ||
            (typeof dif.fallbackLanguage !== "undefined" &&
              dif.fallbackLanguage !== state.preferences.fallbackLanguage)
          ) {
            state.languageContainer = createContainer(
              typeof dif.currentLanguage !== "undefined"
                ? dif.currentLanguage
    ...
    case BackIn.IMPORT_CURATION:
      {
        ...
        try {
          await importCuration({
            ...
            date:
              reqData.date !== undefined ? new Date(reqData.date) : undefined,
...
async function execProcess(
  ...
  try {
    if (sync) {
      ...
      const childProc = child_process.execFile(proc.filename, proc.arguments, {
        cwd: cwd,
      });
...
async function deletePlaylist(
  ...
  if (id && folder !== undefined) {
    ...
    if (playlist) {
      ...
      if (filepath.length > folder.length && filepath.startsWith(folder)) {
