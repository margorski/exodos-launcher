...
import {
    ...
    EditCuration,
    EditCurationMeta,
} from "@shared/curate/types";
import {
    getContentFolderByKey,
    getCurationFolder,
    indexContentFolder,
} from "@shared/curate/util";
...
export async function importCuration(opts: ImportCurationOpts): Promise<void> {
    ...
    const existingGame = GameManager.findGame(
        gameManager.platforms,
        (g) => g.launchCommand === curation.meta.launchCommand,
    );
    if (existingGame) {
        ...
        const response = await opts.openDialog({
            ...
            message:
                "There is already a game using this launch command. It may be a duplicate.\nContinue importing this curation?\n\n" +
    ...
    const contentToMove = [
        [
            getContentFolderByKey(curation.key, fpPath),
            path.join(fpPath, htdocsPath),
        ],
    ];
    const extrasAddApp = curation.addApps.find(
        (a) => a.meta.applicationPath === ":extras:",
    );
    if (
        extrasAddApp &&
        extrasAddApp.meta.launchCommand &&
        extrasAddApp.meta.launchCommand.length > 0
    ) {
        // Add extras folder if meta has an entry
        contentToMove.push([
            path.join(getCurationFolder(curation, fpPath), "Extras"),
            path.join(fpPath, "Extras", extrasAddApp.meta.launchCommand),
        ]);
    }
    // Create and add game and additional applications
    const gameId = uuid();
    const game = createGameFromCurationMeta(gameId, curation.meta, date);
    const addApps = curation.addApps.map((addApp) =>
        createAddAppsFromCurationMeta(gameId, addApp.meta),
    );
    ...
    await GameManager.savePlatforms(gameManager, result.edited).then(() => {
        if (log) {
            logMsg("Meta Added", curation);
        }
    }),
        // Copy Thumbnail
        curationLog(log, "Importing Curation Thumbnail");
    await importGameImage(
        curation.thumbnail,
        game.id,
        LOGOS,
        path.join(fpPath, imagePath),
        log,
    ).then(() => {
        if (log) {
            logMsg("Thumbnail Copied", curation);
        }
    });
    ...
    await importGameImage(
        ...
        if (log) {
            logMsg("Screenshot Copied", curation);
        }
    });
            ...
            try {
                if (saveCuration) {
                    ...
                    const metaPath = path.join(
                        getCurationFolder(curation, fpPath),
                        "meta.yaml",
                    );
                    const meta = YAML.stringify(
                        convertEditToCurationMeta(
                            curation.meta,
                            curation.addApps,
                        ),
                    );
                ...
                curationLog(
                    log,
                    `Error importing ${curation.meta.title} - Informing user...`,
                );
                const res = await opts.openDialog({
                    title: "Error saving curation",
                    message:
                        "Saving curation import failed. Some/all files failed to move. Please check the content folder yourself before removing manually.\n\nOpen folder now?",
                    buttons: ["Yes", "No"],
                });
                if (res === 0) {
 ...
 */
export async function launchCuration(
    key: string,
    meta: EditCurationMeta,
    addAppMetas: EditAddAppCurationMeta[],
    opts: Omit<LaunchGameOpts, "game" | "addApps">,
) {
    ...
    GameLauncher.launchGame({
        ...
        addApps: addAppMetas.map((meta) =>
            createAddAppsFromCurationMeta(key, meta),
        ),
    });
}
 ...
 */
export async function launchAddAppCuration(
    curationKey: string,
    appCuration: EditAddAppCuration,
    opts: Omit<LaunchAddAppOpts, "addApp">,
) {
 ...
 */
function createGameFromCurationMeta(
    gameId: string,
    meta: EditCurationMeta,
    date: Date,
): IGameInfo {
 ...
 * @param curation Curation to get data from.
 */
function createAddAppsFromCurationMeta(
    key: string,
...
async function importGameImage(
    image: CurationIndexImage,
    gameId: string,
    folder: typeof LOGOS | typeof SCREENSHOTS,
    fullImagePath: string,
    log: LogFunc | undefined,
): Promise<void> {
    if (image.exists) {
        ...
        if (imagePath.startsWith(fullImagePath)) {
            // (Make sure the image path does not climb out of the image folder)
...
async function copyFolder(
    ...
    const contentIndex = await indexContentFolder(
        inFolder,
        curationLog.bind(undefined, log),
    );
    let yesToAll = false;
    return Promise.all(
        contentIndex.map(async (content) => {
            ...
            await access(dest, fs.constants.F_OK)
                .then(async () => {
                    ...
                    const filesDifferent = !(await equalFileHashes(
                        source,
                        dest,
                    ));
                    ...
                    if (filesDifferent) {
                        ...
                        const response = await openDialog({
                            ...
                            message:
                                "Overwrite File?\n" +
                                `${content.filePath}\n` +
                                `Current File: ${sizeToString(
                                    currentStats.size,
                                )} (${currentStats.size} Bytes)\n` +
                                `New File: ${sizeToString(newStats.size)} (${
                                    newStats.size
                                } Bytes)`,
...
async function copyOrMoveFile(
    ...
    try {
        ...
        } // @TODO Make sure this overwrites files
        else {
        ...
        curationLog(
            log,
            `Error copying file '${source}' to '${dest}' - ${error.message}`,
        );
        if (move) {
