...
export class LogsPage extends React.Component<LogsPageProps> {
    stringifyLogEntriesMemo = memoizeOne(
        stringifyLogEntries,
        stringifyLogEntriesEquals,
    );
    ...
    onCheckboxClick = (index: number): void => {
        ...
        updatePreferencesData({
            showLogSource: Object.assign({}, showLogSource, {
...
function stringifyLogEntriesEquals(
    newArgs: ArgsType,
    prevArgs: ArgsType,
): boolean {
    return (
        newArgs[0].length === prevArgs[0].length && // (Only compare lengths of log entry arrays)
