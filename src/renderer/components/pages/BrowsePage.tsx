...
> {
	...
	render() {
		...
		if (selectedPlaylistId && selectedGameId) {
			...
			if (playlist) {
				gamePlaylistEntry = playlist.games.find((g) => g.id === selectedGameId);
			}
		}
		// Render
		return (
	...
	onCenterKeyDown = (event: React.KeyboardEvent): void => {
		...
		function updateTime(this: BrowsePage): boolean {
			...
			const timedOut: boolean =
				now - this._prevQuickSearchUpdate > BrowsePage.quickSearchTimeout;
	...
	onPlaylistClick = (playlistId: string, selected: boolean): void => {
		if (!selected) {
			...
			this.props.onSelectPlaylist(
				...
				this.props.selectedPlaylistId !== playlistId ? playlistId : undefined,
			);
		}
	};
