...
> {
        ...
        return (
            <div className="developer-page simple-scroll">
                <div className="developer-page__inner">
                    <h1 className="developer-page__title">
                        {strings.developerHeader}
                    </h1>
                    ...
                    {/* -- Services -- */}
                    <h1 className="developer-page__services-title">
                        {strings.servicesHeader}
                    </h1>
                    {services.length > 0 ? (
                        services.map((item, index) => (
                            <ServiceBox key={index} service={item} />
                        ))
                    ) : (
    ...
    onServiceUpdate = (response: WrappedResponse<ServiceChangeData>) => {
        if (response.type === BackOut.SERVICE_CHANGE) {
            this.forceUpdate();
        }
    };
    ...
    onCheckMissingExecMappings = async (): Promise<void> => {
        const [games, execMappings] = await Promise.all([
            ...
            window.External.back
                .sendP<GetExecData>(BackIn.GET_EXEC, undefined)
                .then((r) => r.data),
        ]);
        if (!execMappings) {
            throw new Error(
                "Failed to get exec mappings - response contained no data",
            );
        }
...
function checkGameIDs(games: IGameInfo[]): string {
    ...
    const invalidIDs: IGameInfo[] = games.filter(
        (game) => !validateSemiUUID(game.id),
    ); // Find all games with invalid IDs
    const timeEnd = Date.now(); // End timing
    // Write log message
    let text = "";
    text += `Checked games for duplicate and invalid IDs (in ${
        timeEnd - timeStart
    }ms)\n`;
    text += "\n";
    text += `Games with duplicate IDs (${Object.keys(dupes).length}):\n`;
    for (let id in dupes) {
        text += `ID: "${id}" | Games (${dupes[id].length}): ${dupes[id]
            .map((game) => `${game.id}`)
            .join(", ")}\n`;
    }
    text += "\n";
    text += `Games with invalid IDs (${invalidIDs.length}):\n`;
    invalidIDs.forEach((game) => {
...
function checkGameTitles(games: IGameInfo[]): string {
    ...
    for (let key in gamesPerPlatform) {
        dupesPerPlatform[key] = checkDupes(gamesPerPlatform[key], (game) =>
    ...
    // Write log message
    let text = "";
    text += `Checked for games with identical titles (case-insensitive) on the same platform (in ${
        timeEnd - timeStart
    }ms)\n`;
    text += "\n";
...
function checkGameEmptyFields(games: IGameInfo[]): string {
    ...
    // Write log message
    let text = "";
    text += `Checked all games for empty fields (in ${
        timeEnd - timeStart
    }ms)\n`;
    text += "\n";
    text += "Summary:\n";
...
function checkPlaylists(playlists: GamePlaylist[], games: IGameInfo[]): string {
    ...
    const invalidIDs: GamePlaylist[] = playlists.filter(
        (playlist) => !uuidValidate(playlist.filename, 4),
    ); // Find all playlists with invalid IDs
    ...
    for (let i = 0; i < playlists.length - 1; i++) {
        ...
        const invalidGameIDs = playlist.games.filter(
            (game) => !validateSemiUUID(game.id),
        ); // Find all games with invalid IDs
        ...
        if (
            Object.keys(duplicateGames).length > 0 ||
    ...
    // Write log message
    let text = "";
    text += `Checked all playlists for duplicate or invalid IDs, and for game entries with invalid, missing or duplicate IDs (in ${
        timeEnd - timeStart
    }ms)\n`;
    text += "\n";
    text += `Playlists with invalid IDs (${invalidIDs.length}):\n`;
    invalidIDs.forEach((playlist) => {
        text += `"${playlist.title}" (ID: ${playlist.filename})\n`;
    });
    text += "\n";
    text += `Playlists with duplicate IDs (${Object.keys(dupes).length}):\n`;
    for (let id in dupes) {
        text += `ID: "${id}" | Playlists (${dupes[id].length}): ${dupes[id]
            .map((playlist) => `${playlist.filename}`)
            .join(", ")}\n`;
    }
    text += "\n";
    text += `Playlists with game entry issues (${reports.length}):\n`;
    reports.forEach(
        ({ playlist, duplicateGames, missingGameIDs, invalidGameIDs }) => {
            text += `  "${playlist.title}" (ID: ${playlist.filename}):\n`;
            // Log duplicate game entry IDs
            if (Object.keys(duplicateGames).length > 0) {
                text += `    Game entries with duplicate IDs (${
                    Object.keys(duplicateGames).length
                }):\n`;
...
function checkMissingExecMappings(
    games: IGameInfo[],
    ...
    for (let i = 0; i < games.length; i++) {
        const game = games[i];
        if (
            allExecs.findIndex((exec) => {
                return exec === game.applicationPath;
            }) === -1
        ) {
    ...
    for (let i = 0; i < allExecs.length; i++) {
        ...
        if (
            execMappings.findIndex((mapping) => {
                return mapping.win32 === exec;
            }) === -1
        ) {
            text += `    ${exec}\n`;
        }
    ...
    for (let i = 0; i < allExecs.length; i++) {
        ...
        if (
            execMappings.findIndex((mapping) => {
                return mapping.win32 === exec && mapping.linux;
            }) === -1
        ) {
            text += `    ${exec}\n`;
        }
    ...
    for (let i = 0; i < allExecs.length; i++) {
        ...
        if (
            execMappings.findIndex((mapping) => {
                return mapping.win32 === exec && mapping.darwin;
            }) === -1
        ) {
            text += `    ${exec}\n`;
        }
 ...
 */
function categorizeByProp<
    T extends Map<K, V>,
    K extends string,
    V extends string,
>(array: T[], prop: K): Map<string, T[]> {
...
function checkFileLocation(games: IGameInfo[]): string {
    ...
    // Write log message
    let text = "";
    text += `Checked all games to see if their "launch command" could be parsed into a file path (in ${
        timeEnd - timeStart
    }ms)\n`;
    text += "\n";
    text += `Total games that failed: ${
        pathFailed.length + pathError.length
    }\n`;
    ...
    for (let [game, error] of pathError) {
        text +=
            `"${game.title}" (Platform: "${game.platform}", ID: "${game.id}")\n` +
            `    ${error.toString()}\n`;
    }
    // Done
    return text;
        ...
        async function createMissingFolder(
            ...
            if (!(await exists(folderPath))) {
                ...
                if (folderPath.startsWith(rootPath)) {
                    // Folder is a sub-folder of it's root (no "../" climbing allowed)
...
export function removeLastItemOfPath(filePath: string): string {
    return filePath.substr(
        0,
        Math.max(0, filePath.lastIndexOf("/"), filePath.lastIndexOf("\\")),
    );
}
...
function fetchAllGames(): Promise<IGameInfo[]> {
    return new Promise((resolve, reject) => {
        window.External.back.send<GetAllGamesResponseData>(
            BackIn.GET_ALL_GAMES,
            undefined,
            (result) => {
                if (result.data) {
                    resolve(result.data.games);
                } else {
                    reject(
                        new Error(
                            "Failed to fetch all games. Data is undefined.",
                        ),
                    );
                }
            },
