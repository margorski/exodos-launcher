...
> {
    ...
    onBrokenChange = this.wrapOnCheckBoxChange((game) => {
        game.broken = !game.broken;
    });
    onExtremeChange = this.wrapOnCheckBoxChange((game) => {
        game.extreme = !game.extreme;
    });
    ...
    componentDidUpdate(
        prevProps: RightBrowseSidebarProps,
        prevState: RightBrowseSidebarState,
    ): void {
    ...
    render() {
        ...
        if (game) {
            ...
            const playButtonLabel = isDosPlatform
                ? isInstalled
                    ? strings.play
                    : strings.install
                : strings.open;
                ...
                <div
                    className={
                        ...
                        (editable
                            ...
                            : "browse-right-sidebar--edit-disabled")
                    }
    ...
    onGlobalKeyDown = (event: KeyboardEvent) => {
        ...
        if (
            event.ctrlKey &&
            event.code === "KeyE" && // (CTRL + E ...
            !this.props.isEditing &&
            this.props.currentGame
        ) {
            // ... while not editing, and a game is selected)
            this.props.onEditClick();
            if (this.launchCommandRef.current) {
                this.launchCommandRef.current.focus();
            }
    ...
    onLocalKeyDown = (event: React.KeyboardEvent) => {
        ...
        if (
            ...
            // ... while editing, and a game is selected)
    ...
    addImageDialog(folder: typeof LOGOS | typeof SCREENSHOTS) {
        return () => {
            ...
            if (!currentGame) {
                throw new Error(
                    "Failed to add image file. The currently selected game could not be found.",
                );
            }
            ...
            if (filePaths && filePaths[0]) {
                fs.readFile(filePaths[0], (error, data) => {
                    if (error) {
                        ...
                        window.External.back.send<any, SaveImageData>(
                            BackIn.SAVE_IMAGE,
                            {
                                folder: folder,
                                id: currentGame.id,
    ...
    imageDrop(
        type: typeof LOGOS | typeof SCREENSHOTS,
        ...
        return (event) => {
            ...
            if (!currentGame) {
                throw new Error(
                    'Can not add a new image, "currentGame" is missing.',
                );
            }
            if (files.length > 1) {
                // (Multiple files)
                    ...
                    if (reader.result && typeof reader.result === "object") {
                        window.External.back.send<any, SaveImageData>(
                            BackIn.SAVE_IMAGE,
                            {
                                folder: folder,
                                id: id,
                                content: Buffer.from(reader.result).toString(
                                    "base64",
                                ),
                            },
                        );
            ...
            function copyArrayLike<T>(arrayLike: {
                [key: number]: T;
            }): Array<T> {
                const array: T[] = [];
    ...
    onNewAddAppClick = (): void => {
        if (!this.props.currentAddApps) {
            throw new Error(
                'Unable to add a new AddApp. "currentAddApps" is missing.',
            );
        }
        if (!this.props.currentGame) {
            throw new Error(
                'Unable to add a new AddApp. "currentGame" is missing.',
            );
        }
