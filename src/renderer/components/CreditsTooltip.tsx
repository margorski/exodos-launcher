...
export function CreditsTooltip(props: CreditsTooltipProps) {
    ...
    return (
        ...
        >
            {props.profile ? (
                <>
                    <p className="about-page__credits__tooltip__title">
                        {props.profile.title}
                    </p>
                    {props.profile.note ? (
                        <p className="about-page__credits__tooltip__note">
                            {props.profile.note}
                        </p>
                    ) : undefined}
    ...
    return (event) => {
        if (current) {
            if (event.clientX <= window.innerWidth * 0.5) {
                ...
                current.style.left = "";
                current.style.right =
                    window.innerWidth - event.clientX + 16 + "px";
            }
...
function getRoleColor(role: string): string | undefined {
    ...
    switch (role) {
        ...
        case "The Moe":
            return "rgb(224, 164, 241)";
        case "Administrator":
            return "rgb(52, 152, 219)";
        case "Hacker":
            return "rgb(177, 21, 21)";
        case "Archivist":
            return "rgb(170, 135, 135)";
        case "Tester":
            return "rgb(230, 126, 34)";
        case "VIP":
            return "rgb(214, 4, 127)";
    }
}
