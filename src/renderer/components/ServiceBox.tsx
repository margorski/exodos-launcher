...
export function ServiceBox(props: ServiceBoxProps) {
    ...
    useInterval(
        () => {
            // (Update the value of the timer at an interval)
    ...
    // Button callbacks
    const onStopClick = useProcessActionCallback(
        ProcessAction.STOP,
        service.id,
    );
    const onStartClick = useProcessActionCallback(
        ProcessAction.START,
        service.id,
    );
    const onRestartClick = useProcessActionCallback(
    ...
    return (
        <div className="service-box">
            ...
            <div className="service-box__head-top">
                <div className="service-box__title">{service.name}</div>
                <div className="service-box__status">{statusText}</div>
            </div>
            {/* Uhm */}
            <div className="service-box__head-bottom">
                ...
                <div className="service-box__uptime" ref={uptimeRef}>
                    {""}
                </div>
            </div>
            {/* Log */}
...
function generateStatusText(
    service: IService,
    lang: LangContainer["developer"],
): string {
    switch (service.state) {
        default:
            throw new Error(
                "Failed to generate status text. Unexpected process state value.",
            );
 ...
 */
function useProcessActionCallback(
    action: ProcessAction,
    id: string,
): () => void {
    return React.useCallback(() => {
        window.External.back.send<any, ServiceActionData>(
            BackIn.SERVICE_ACTION,
            {
                id,
                action,
            },
 ...
 */
function stringifyServiceLogEntries(
    entries: ILogEntry[],
    source: string,
): string {
    let str = "";
    for (let i = 0; i < entries.length; i++) {
        ...
        if (entry.source === source) {
            str += `<span class="log__time-stamp">[${formatTime(
