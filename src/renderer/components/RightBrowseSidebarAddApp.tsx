...
export class RightBrowseSidebarAddApp extends React.Component<RightBrowseSidebarAddAppProps> {
    onNameEditDone = this.wrapOnTextChange((addApp, text) => {
        addApp.name = text;
    });
    onApplicationPathEditDone = this.wrapOnTextChange((addApp, text) => {
        addApp.applicationPath = text;
    });
    onLaunchCommandEditDone = this.wrapOnTextChange((addApp, text) => {
        addApp.launchCommand = text;
    });
    onAutoRunBeforeChange = this.wrapOnCheckBoxChange((addApp) => {
        addApp.autoRunBefore = !addApp.autoRunBefore;
    });
    onWaitForExitChange = this.wrapOnCheckBoxChange((addApp) => {
        addApp.waitForExit = !addApp.waitForExit;
    });
    ...
    renderDeleteButton({
        ...
        reset,
        extra,
    }: ConfirmElementArgs<LangContainer["browse"]>): JSX.Element {
        const className =
        ...
        return (
            <div
                className={
                    className +
                    (activationCounter > 0
                        ? ` ${className}--active simple-vertical-shake`
                        : "")
                }
    ...
    wrapOnTextChange(
        func: (addApp: IAdditionalApplicationInfo, text: string) => void,
    ): (
