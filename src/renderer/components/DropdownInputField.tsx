...
> {
        ...
        if (editable) {
            return (
                <div
                    className={
                        "input-dropdown" +
                        (this.props.disabled ? " input-dropdown--disabled" : "")
                    }
                    ref={this.rootRef}
                    onBlur={this.onBlur}
                >
                    ...
                    <div
                        className={
                            "input-dropdown__content simple-scroll" +
                            (expanded ? "" : " input-dropdown__content--hidden")
                        }
    ...
    onListItemKeyDown = (event: React.KeyboardEvent): void => {
        if (!this.props.disabled) {
            ...
            if (key === "ArrowUp" || key === "ArrowDown") {
                ...
                if (
                    element &&
                    checkIfAncestor(element, this.contentRef.current)
                ) {
                    const next: any =
                        key === "ArrowUp"
    ...
    onBlur = (event: React.FocusEvent): void => {
        ...
        if (
            relatedTarget &&
            !checkIfAncestor(relatedTarget as any, this.rootRef.current)
        ) {
            this.setState({ expanded: false });
        }
    };
    ...
    onInputKeyDown = (event: React.KeyboardEvent<InputElement>): void => {
        if (!this.props.disabled) {
            ...
            if (key === "ArrowUp" || key === "ArrowDown") {
                ...
                if (!content) {
                    throw new Error(
                        "dropdown input field content div is missing",
                    );
                }
                const element: any =
                    key === "ArrowUp" ? content.lastChild : content.firstChild;
