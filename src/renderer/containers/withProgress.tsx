...
export function withProgress<T extends WithProgressProps>(
    Component: React.ComponentType<T>,
) {
    return function WithProgress(props: Subtract<T, WithProgressProps>) {
        // Holds state used internally
        const [itemCount, setItemCount] = React.useReducer(
            itemCountReducer,
            initialItemsCount,
        );
        ...
        const setIsDoneCallback = useCallback(
            (isDone: boolean = true) => {
                setIsDone(isDone);
            },
            [setIsDone],
