...
const langTemplate = {
    ...
    logs: ["filters", "copyText", "clearLog"] as const,
    app: [
        "home",
        "browse",
        "logs",
 ...
 */
function createCategory<T extends readonly string[]>(
    tempCat: T,
): LangCategory<T> {
...
function createBaseLangContainer(): BaseLangContainer {
    ...
    for (let key in langTemplate) {
        lang[key as keyof LangTemplate] = createCategory(
            langTemplate[key as keyof LangTemplate],
        );
    }
    return lang;
}
...
export function getDefaultLocalization(): LangContainer {
    ...
    lang.dialog.errorParsingPlatformsMessage =
        "{0} " + lang.dialog.errorParsingPlatformsMessage;
    lang.dialog.upgradeWillInstallTo =
        "{0} " + lang.dialog.upgradeWillInstallTo;
    // Return object
    return lang;
}
