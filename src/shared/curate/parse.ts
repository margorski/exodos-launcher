...
export function convertMeta(
    ...
    const parser = new ObjectParser({
        ...
        onError:
            ...
            ((e) =>
                onError(
                    `Error while converting Curation Meta: ${e.toString()}`,
                )),
    });
    // -- Old curation format --
    ...
    parser.prop(
        "application path",
        (v) => (parsed.game.applicationPath = str(v)),
    );
    ...
    parser.prop(
        ...
        (v) => (parsed.game.originalDescription = str(v)),
    );
    ...
    parser.prop(
        "alternate titles",
        (v) => (parsed.game.alternateTitles = arrayStr(v)),
    );
 ...
 */
function convertAddApp(
    item: IObjectParserProp<any>,
    ...
    switch (labelStr.toLowerCase()) {
        ...
        default: // (Normal add-app)
            ...
            item.prop(
                "Application Path",
                (v) => (addApp.applicationPath = str(v)),
            );
...
export function generateExtrasAddApp(
    folderName: string,
): EditAddAppCurationMeta {
    return {
        heading: "Extras",
