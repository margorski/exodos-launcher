        ...
        if (line[0] === "#") {
            ...
            if (state.inMultiLine) {
                ...
                if (indent !== state.indent) {
                    ...
                    if (
                        prevToken &&
                        prevToken.type === CFTokenizer.TokenType.INDENT_CHANGE
                    ) {
            ...
            if (state.inMultiLine) {
                ...
                state.multiLineChunks.push(
                    line
                        .substring(
                            countIndentChars(line, state.multiLineIndent),
                        )
                        .trimRight(),
                );
...
function countSpacesLeft(str: string): number {
    ...
    characterLoop: for (let i = 0; i < str.length; i++) {
        switch (str[i]) {
            // A space character
            case " ":
            case "\t":
