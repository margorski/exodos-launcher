 ...
 */
export function parseCurationFormat(
    tokens: CFTokenizer.AnyToken[],
): CurationFormatObject {
...
function parseObject(
    ...
    // (Label used to make it possible to exit the loop while in a nested loop)
    mainLoop: while (index < tokens.length) {
                            ...
                            case CFTokenizer.TokenType.LIST_ITEM:
                                {
                                    ...
                                    for (
                                        ...
                                        if (
                                            itemToken.type ===
                                            CFTokenizer.TokenType.LIST_ITEM
                                        ) {
                            ...
                            case CFTokenizer.TokenType.INDENT_CHANGE:
                                ...
                                if (nextToken.delta === 1) {
                                    ...
                                    if (childState.indentOverflow > 0) {
                                        // Set the indent overflow
                                        state.indentOverflow =
                                            childState.indentOverflow - 1;
                                        // End the current object
                                        break mainLoop; // (Exit the main loop)
                                    }
                                }
 ...
 */
function indexOfNonComment(
    tokens: CFTokenizer.AnyToken[],
    startIndex: number = 0,
): number {
    for (let i = startIndex; i < tokens.length; i++) {
        if (tokens[i].type !== CFTokenizer.TokenType.COMMENT) {
            return i;
        }
    }
    return -1;
